...
export interface MultiPOExtractResponse {
...
export interface SKULookupResponse {
  ...
  // New fields from the all_sku-backed endpoint (2026 update).
  // uom is serialized as string to preserve decimal precision (e.g. "0.400").
...
class InwardApiService {
  ...
  async extractPO(file: File): Promise<MultiPOExtractResponse> {
  ...
  async getInwardList(
    ...
    if (params?.warehouse) sp.append("warehouse", params.warehouse)
    if (params?.approval_authority) sp.append("approval_authority", params.approval_authority)
    if (params?.has_approval_data !== undefined) sp.append("has_approval_data", params.has_approval_data.toString())
    if (params?.has_po !== undefined) sp.append("has_po", params.has_po.toString())
  ...
  // ── PO Upload Status (7-day window) ────────────────────
  // Counts distinct POs seen on inward records per day — not raw inward count.
  // If an inward has no po_number it's skipped. "Uploaded POs without any
  // inward yet" require a dedicated backend endpoint and aren't included here.
  async getPoUploadStatus(company: Company): Promise<{ date: string; count: number }[]> {
    ...
    const response = await this.getInwardList(company, {
      per_page: 500,
      from_date: fromDate,
      to_date: toDate,
    })

    const poByDate = new Map<string, Set<string>>()
    for (let i = 0; i < 7; i++) {
      const d = new Date(today)
      d.setDate(d.getDate() - i)
      poByDate.set(d.toISOString().split("T")[0], new Set())
    }

    for (const record of response.records) {
      if (!record.entry_date || !record.po_number) continue
      const dateKey = record.entry_date.split("T")[0]
      if (!poByDate.has(dateKey)) poByDate.set(dateKey, new Set())
      poByDate.get(dateKey)!.add(record.po_number)
    }

    return Array.from(poByDate.entries())
      .map(([date, set]) => ({ date, count: set.size }))
      .sort((a, b) => b.date.localeCompare(a.date))
  }

  // ── PO Breakdown — distinct POs seen across inwards in a window ───
  // Returns one entry per unique po_number with aggregates. Covers the same
  // visibility limitation as getPoUploadStatus: POs uploaded but never
  // referenced on any inward do NOT appear here. A dedicated PO listing
  // endpoint would be needed for the true "pending PO" view.
  async getPoBreakdown(
    company: Company,
    daysBack = 30,
  ): Promise<Array<{
    po_number: string
    ...
    const byPo = new Map<string, { inward_count: number; total_amount: number; latest_entry_date: string | null; vendor: string | null }>()
    for (const r of response.records) {
      if (!r.po_number) continue
      const entry = byPo.get(r.po_number) || { inward_count: 0, total_amount: 0, latest_entry_date: null, vendor: null }
      entry.inward_count++
      entry.total_amount += r.total_amount || 0
      if (r.entry_date && (!entry.latest_entry_date || r.entry_date > entry.latest_entry_date)) {
        entry.latest_entry_date = r.entry_date
      }
      if (!entry.vendor && r.vendor_supplier_name) entry.vendor = r.vendor_supplier_name
      byPo.set(r.po_number, entry)
    }
    return Array.from(byPo.entries())
      .map(([po_number, v]) => ({ po_number, ...v }))
      .sort((a, b) => (b.latest_entry_date || "").localeCompare(a.latest_entry_date || ""))
  }
  ...
  async getWarehouseList(company: Company): Promise<string[]> {
    ...
    const data = await handleResponse<{ warehouses: string[] }>(response)
...
export const getWarehouseList = inwardApiService.getWarehouseList.bind(inwardApiService)
