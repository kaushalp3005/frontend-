  ...
  const updateArticle = (id: string, field: string, value: any) => {
    setArticles(prev => prev.map((article) => {
      if (article.id === id) {
        ...
        if (!skipRecalc && ["quantity_units", "pack_size", "unit_pack_size", "material_type"].includes(field)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isColdInvolved) {
        ...
        // Build per-item summary from articles
        const itemLines = articles.map((art) =>
          `Item Mark : -\nNo of Boxes : ${art.quantity_units || 0}\nLot Number : ${art.lot_number || "-"}`
        )
        // If scannedBoxes exist, build from those instead (more accurate)
        if (scannedBoxes.length > 0) {
          ...
          Object.values(itemMap).forEach((item) => {
            itemLines.push(`Item Mark : ${item.itemMark}\nNo of Boxes : ${item.boxes}\nLot Number : ${item.lotNumber}`)
          })
        }
        itemLines.push(`From : ${fromWh || "-"}\nTo : ${toWh || "-"}`)
        itemLines.push(`Vehicle Number : ${vehicleNo || "-"}`)
        const message = itemLines.join("\n\n")
