  ...
  useEffect(() => {
    const fetchSkuIdForAutoFilledArticle = async () => {
      ...
      if (articles[0]?.material_type &&
        ...
        // Check if any dropdown field is "other" - skip SKU lookup for these items
        const fieldsToCheck = [
          articles[0].item_description,
          articles[0].item_category,
          articles[0].sub_category,
          articles[0].material_type
        ]
        const isOtherItem = fieldsToCheck.some(
          field => typeof field === 'string' && field.toLowerCase() === 'other'
        )

        if (isOtherItem) {
          console.log('⏭️ Skipping SKU lookup for "other" item - no real SKU exists')
          return
        }
      ...
      <Card className="w-full bg-gray-50 border-gray-200">
        ...
        <CardContent className="pt-0 bg-gray-50">
          {!showScanner ? (
            ...
            <div className="py-4">
              <div className="h-[340px] sm:h-[440px] md:h-[520px] lg:h-[560px] rounded-lg overflow-hidden">
