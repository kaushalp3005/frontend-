...
export default function BulkStickerPage({ params }: BulkStickerPageProps) {
  ...
  const [result, setResult] = useState<BulkEntryCreateResponse | null>(null)
  ...
  const generateTxnNo = (d: Date) =>
    `BE-${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}${String(d.getDate()).padStart(2, "0")}${String(d.getHours()).padStart(2, "0")}${String(d.getMinutes()).padStart(2, "0")}${String(d.getSeconds()).padStart(2, "0")}`
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload: BulkEntryCreatePayload = {
        company,
        transaction: {
      ...
      const response = await bulkEntryApi.create(payload)
      setResult(response)
      toast({
  ...
  const handleAddMoreBoxes = async (articleIdx: number) => {
    ...
    try {
      ...
      for (let i = 0; i < qty; i++) {
        ...
        const res = await bulkEntryApi.upsertBox(company, result.transaction_no, {
  ...
  const handlePrintArticle = async (articleGroup: BulkEntryArticleResponse) => {
    if (!result) return
    const af = articleForms.find((a) => a.item_description === articleGroup.article_description)
  ...
  if (result) {
    return (
      <PermissionGuard module="cold-storage" action="view" showError>
        <div className="p-3 sm:p-4 md:p-6 max-w-[900px] mx-auto space-y-3 sm:space-y-4">
          {/* Header */}
          <div className="flex items-center gap-2 sm:gap-3">
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => router.push(`/${company}/cold-storage`)}>
          ...
          <div className="flex justify-center pt-2">
            <Button variant="outline" onClick={() => router.push(`/${company}/cold-storage`)}>
    ...
    <PermissionGuard module="cold-storage" action="view" showError>
      <div className="p-3 sm:p-4 md:p-6 max-w-[900px] mx-auto space-y-3 sm:space-y-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold tracking-tight">Cold Storage — Bulk Sticker Entry</h1>
        ...
        <AlertDialog open={showDiscard} onOpenChange={setShowDiscard}>
          <AlertDialogContent className="max-w-[90vw] sm:max-w-lg">
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={() => router.push(`/${company}/cold-storage`)}>Discard</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
