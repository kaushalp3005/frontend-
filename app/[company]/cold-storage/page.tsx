...
import {
  ...
  Loader2, Package, Printer, BarChart3, LogOut,
} from "lucide-react"
...
import { getColdWarehouseCodes, getUserDefaultWarehouses, normalizeWarehouseName } from "@/lib/constants/warehouses"
...
export default function ColdStorageListPage({ params }: ColdStorageListPageProps) {
  ...
  const [coldUserDefaults, setColdUserDefaults] = useState<string[]>([])

  // Set default warehouse filter based on user
  useEffect(() => {
    if (user?.name) {
      const defaults = getUserDefaultWarehouses(user.name)
      const coldDefaults = defaults.filter((d) => getColdWarehouseCodes().includes(d))
      setColdUserDefaults(coldDefaults)
      if (coldDefaults.length === 1) {
        setWarehouseFilter(coldDefaults[0])
      } else if (coldDefaults.length > 1) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const filteredRecords = (() => {
        if (warehouseFilter === "my_warehouses" && coldUserDefaults.length > 0) {
          const set = new Set(coldUserDefaults)
          return records.filter((r) => r.warehouse && set.has(r.warehouse))
        }
        // If specific warehouse, client-side filter too (since backend may not)
        if (warehouseFilter !== "all" && warehouseFilter !== "my_warehouses") {
          return records.filter((r) => r.warehouse === warehouseFilter)
        }
        return records
      })()
      setEntries(filteredRecords)
  ...
  }, [company, currentPage, perPage, searchQuery, fromDate, toDate, warehouseFilter])
  ...
  useEffect(() => { setCurrentPage(1) }, [searchQuery, fromDate, toDate, warehouseFilter])
      ...
      <div className="p-3 sm:p-4 md:p-6 max-w-[1400px] mx-auto space-y-3 sm:space-y-4">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <Button asChild size="sm" className="gap-1.5 bg-orange-600 hover:bg-orange-700 text-white">
              <Link href={`/${company}/cold-storage/direct-out`}>
                <LogOut className="h-4 w-4" />
                <span>Direct OUT</span>
              </Link>
            </Button>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select value={warehouseFilter} onValueChange={setWarehouseFilter}>
              ...
              <SelectContent>
                ...
                {coldUserDefaults.length > 1 && (
                  <SelectItem value="my_warehouses">My Warehouses ({coldUserDefaults.length})</SelectItem>
                )}
                    ...
                    <tbody>
                      {entries.map((entry) => (
                        <tr key={entry.transaction_no} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                          ...
                          <td className="px-4 py-3 hidden lg:table-cell text-muted-foreground">{normalizeWarehouseName(entry.warehouse) || "\u2014"}</td>
                          <td className="px-4 py-3">
                  ...
                  {entries.map((entry) => (
                    <div key={entry.transaction_no} className="p-3 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1 text-xs text-muted-foreground">
                        ...
                        {entry.warehouse && (<><span>{"\u00b7"}</span><span>{normalizeWarehouseName(entry.warehouse)}</span></>)}
                      </div>
