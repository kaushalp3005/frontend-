...
export default function NewInwardPage({ params }: NewInwardPageProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Wait for backend to fully persist the entry before approving
  ...
  const handleSubmitMulti = async () => {
    try {
      ...
      // Validate mandatory fields for each PO
      const missing: string[] = []
      editablePOs.forEach((po, idx) => {
        const label = `PO ${idx + 1}`
        if (!po.supplier_name?.trim()) missing.push(`${label} — Vendor / Supplier`)
        if (!po.customer_name?.trim()) missing.push(`${label} — Customer / Party`)
        if (!po.source_location?.trim()) missing.push(`${label} — Source Location`)
        if (!po.destination_location?.trim()) missing.push(`${label} — Destination Location`)
        if (!po.purchased_by?.trim()) missing.push(`${label} — Indentor`)
        po.articles.forEach((a, aIdx) => {
          if (!a.item_description?.trim()) missing.push(`${label} Article ${aIdx + 1} — Item Description`)
        })
      })
      if (missing.length > 0) {
        const message = `Please fill in: ${missing.join(", ")}`
  ...
  const handlePrintBox = async (boxIdx: number) => {
    ...
    try {
      ...
      // Wait for backend to fully persist the entry
