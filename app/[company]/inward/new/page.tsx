...
export default function NewInwardPage({ params }: NewInwardPageProps) {
  ...
  // Print range state (per article index)
  const [printRangeFrom, setPrintRangeFrom] = useState<Record<number, string>>({})
  const [printRangeTo, setPrintRangeTo] = useState<Record<number, string>>({})
  const [printingRange, setPrintingRange] = useState<number | null>(null)
    ...
    const payload: CreateInwardPayload = {
      ...
      articles: articles.map((a, idx) => {
        ...
        return {
          ...
          box_count: parseInt(approvalForm?.box_count) || 1,
          box_net_weight: r3(approvalForm?.box_net_weight),
          box_gross_weight: r3(approvalForm?.box_gross_weight),
        }
      }),
      boxes: articles.flatMap((a, idx) => {
        const approvalForm = articleApprovalForms[idx]
        ...
        return Array.from({ length: count }, (_, i) => ({
          ...
          gross_weight: r3(approvalForm?.box_gross_weight),
          lot_number: approvalForm?.lot_number || undefined,
        }))
      }),
    }
  ...
  // ── Print range of boxes for an article ────────────────────
  const handlePrintRange = async (articleIdx: number, overrideFrom?: number, overrideTo?: number) => {
    ...
    const from = overrideFrom ?? (parseInt(printRangeFrom[articleIdx]) || 1)
    const to = overrideTo ?? (parseInt(printRangeTo[articleIdx]) || articleBoxes.length)
    const rangeBoxes = articleBoxes.filter(({ box }) => box.box_number >= from && box.box_number <= to)

    if (rangeBoxes.length === 0) {
      toast({ title: "Invalid range", description: `No boxes found in range ${from}–${to}.`, variant: "destructive" })
      return
    }

    try {
      setPrintingRange(articleIdx)
      setPrintProgress(`Saving ${rangeBoxes.length} boxes...`)

      const txnNo = await ensureEntryCreated()

      // Upsert all boxes in the range
      ...
      for (const { box, idx } of rangeBoxes) {
        ...
        // Mark as printed
        setBoxForms((prev) =>
          prev.map((b, i) => (i === idx ? { ...b, box_id: upsertResult.box_id, is_printed: true } : b))
        )
      }

      // Generate all QR codes in parallel
      setPrintProgress(`Generating ${upsertedBoxes.length} labels...`)
      const qrCodes = await Promise.all(
        upsertedBoxes.map((box) =>
      ...
      const labelsHtml = upsertedBoxes.map((box, i) => `
        <div class="label">
      ...
      setPrintProgress(`Printing ${upsertedBoxes.length} labels...`)
      ...
      setPrintProgress("")
      toast({ title: "Print Complete", description: `Sent ${upsertedBoxes.length} labels (Box #${from}–#${to}) to printer.` })
    } catch (err) {
      console.error("Print range failed:", err)
      toast({
        title: "Print failed",
        description: err instanceof Error ? err.message : "Failed to print labels",
        variant: "destructive",
      })
    } finally {
                    ...
                    <div key={idx} className="space-y-3 p-3 sm:p-4 border rounded-lg">
                      ...
                      {approvalForm && (
                        <>
                          ...
                          {/* Box configuration — create multiple boxes with same weight */}
                          <div className="grid grid-cols-3 gap-3 pt-2 border-t bg-muted/30 -mx-3 px-3 py-3 rounded-b-lg mt-2">
                            <div className="space-y-1">
                              <Label className="text-[11px] font-semibold">Box Count <span className="text-destructive">*</span></Label>
                              <Input
                                type="number"
                                min="1"
                                value={approvalForm.box_count}
                                onChange={(e) => {
                                  updateArticleApproval(idx, "box_count", e.target.value)
                                  handleQuantityUnitsChange(idx, e.target.value)
                                }}
                                className="h-8 text-xs"
                              />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[11px] font-semibold">Box Net Weight</Label>
                              <Input
                                type="number"
                                step="0.001"
                            ...
                            <div className="space-y-1">
                              <Label className="text-[11px] font-semibold">Box Gross Weight</Label>
                              <Input
                                type="number"
                                step="0.001"
                                value={approvalForm.box_gross_weight}
                                onChange={(e) => {
                          ...
                          <div className="flex items-end gap-2 flex-wrap mt-2 pt-2 border-t">
                            <div className="space-y-1">
                              ...
                              <Input
                                ...
                                min="1"
                                placeholder="1"
                                value={printRangeFrom[idx] || ""}
                                onChange={(e) => setPrintRangeFrom((prev) => ({ ...prev, [idx]: e.target.value }))}
