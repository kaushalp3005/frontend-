      ...
      try {
        ...
        if (data.options && data.options.material_types && Array.isArray(data.options.material_types)) {
          ...
          const fallbackOptions = [
            ...
            { value: "RM", label: "RM (Raw Materials)" },
            { value: "PACKAGING MATERIALS", label: "PACKAGING MATERIALS" },
            { value: "PM", label: "PM (Packaging Materials)" },
            { value: "SPARE PARTS", label: "SPARE PARTS" },
            { value: "SP", label: "SP (Spare Parts)" }
          ]
...
function SubCategoryDropdown({ categoryId, value, onValueChange, company, error, disabled, materialType, otherValue, onOtherValueChange }: SubCategoryDropdownProps & { materialType?: string; otherValue?: string; onOtherValueChange?: (value: string) => void }) {
...
function ItemDescriptionDropdown({
  ...
  otherValue,
  onOtherValueChange
}: ItemDescriptionDropdownProps & { materialType: string; otherValue?: string; onOtherValueChange?: (value: string) => void }) {
...
export default function InwardFormPage({ params }: InwardFormPageProps) {
  ...
  const generateBoxes = (articlesToUse?: Article[], forceRecalculate: boolean = false, articleId?: string) => {
    ...
    articlesForGeneration.forEach((article) => {
      ...
      if (article.uom === "BOX" || article.uom === "CARTON" || article.uom === "BAG") {
  ...
  const updateBox = useCallback((boxId: string, field: keyof Box, value: any) => {
    if (field === "net_weight" || field === "gross_weight" || field === "count") {
      // Default to 0 for empty weight values
      const finalValue = (field === "net_weight" || field === "gross_weight")
    ...
    try {
      ...
      const submissionData = {
        ...
        articles: resolvedArticles.map(article => ({
          ...
          material_type: article.material_type.toLowerCase() === "other" ? article.material_type_other : article.material_type,
          item_description: article.item_description.toLowerCase() === "other" ? article.item_description_other : article.item_description,
          item_category: article.item_category.toLowerCase() === "other" ? article.item_category_other : article.item_category,
          sub_category: article.sub_category.toLowerCase() === "other" ? article.sub_category_other : article.sub_category,
          uom: article.uom,
          packaging_type: article.packaging_type,
          quantity_units: article.quantity_units,
          net_weight: article.net_weight,
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input 
                    ...
                    onChange={(e) => updateFormData("po_number", e.target.value)} 
                    placeholder="PO001"
                  />
                </div>
                <div>
