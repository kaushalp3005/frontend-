...
export interface DropdownResponse {
  ...
  options: {
    ...
    item_ids?: number[]
    // Arrays parallel-indexed with item_ids / item_descriptions (2026 update):
    // item_ids[i] <-> item_descriptions[i] <-> item_sale_groups[i] <-> item_uoms[i]
    item_sale_groups?: string[]
    item_uoms?: string[]
  }
  meta: {
...
export const dropdownApi = {
  async fetchDropdown(params: {
    ...
    const { company, material_type, item_category, sub_category, item_description, search, limit, offset } = params
    ...
    if (item_description) query.append('item_description', item_description)
  ...
  async fetchSkuId(params: {
    ...
    // uom serialized as string for decimal precision (e.g. "0.400"), from 2026 spec.
    uom?: string | null
    company?: string
  }> {
  ...
  async globalSearch(params: {
    ...
    items: Array<{
      ...
      sub_group?: string
      sale_group?: string | null
      // uom serialized as string to preserve decimal precision (2026 update).
      uom?: string | null
    }>
    meta: { total_items: number; limit: number; offset: number; search: string | null; has_more: boolean }
  }> {
    const { company, search, limit = 10000, offset = 0 } = params
  ...
  async skuDropdown(params: {
    ...
    options: {
      ...
      item_descriptions: string[]
      item_ids: number[]
      // Parallel-indexed with item_ids / item_descriptions (2026 update):
      // item_ids[i] <-> item_descriptions[i] <-> item_sale_groups[i] <-> item_uoms[i]
      item_sale_groups?: string[]
      item_uoms?: string[]
    }
  }> {
    const { company, material_type, item_category, sub_category, item_description, search } = params
  ...
  async getVendors(search?: string): Promise<Array<{ id: number; vendor_name: string; location: string | null }>> {
    const query = new URLSearchParams()
    if (search) query.append('search', search)

    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000'}/inward/vendors?${query.toString()}`
