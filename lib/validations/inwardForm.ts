...
export class InwardFormValidator {
   ...
   * @param isOtherItem - If true, null/0 SKU IDs are allowed (for "other" items)
   */
  static validateSkuId(skuId: number | null | undefined, isOtherItem: boolean = false): ValidationResult {
    // "Other" items don't have a real SKU, so null is valid for them
    if (isOtherItem) {
      return {
        isValid: true,
        errors: [],
