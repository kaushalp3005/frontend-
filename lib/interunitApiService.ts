...
export class InterunitApiService {
  ...
  static async createRequest(
    ...
    const userEmail = createdBy || useAuthStore.getState().user?.email || 'unknown'
    const url = `${API_BASE_URL}/interunit/requests?created_by=${encodeURIComponent(userEmail)}`
  ...
  static async getRequests(params?: {
    ...
    // Backend now returns paginated response: { records, total, page, per_page, total_pages }
    return response as RequestListResponse
  }
  ...
  static async submitTransfer(company: string, payload: any, createdBy?: string): Promise<any> {
    ...
    try {
      ...
      const userEmail = createdBy || useAuthStore.getState().user?.email || 'unknown'
      const url = `${API_BASE_URL}/interunit/transfers?created_by=${encodeURIComponent(userEmail)}`
  ...
  static async createPendingTransferIn(payload: {
    ...
    return await fetchJSON(`${API_BASE_URL}/interunit/transfer-in/pending`, {
      method: 'POST',
      body: JSON.stringify(payload)
    })
  }

  static async acknowledgeBox(headerId: number, payload: {
    box_id: string
  ...
  static async unacknowledgeBox(headerId: number, boxId: string): Promise<any> {
    return await fetchJSON(`${API_BASE_URL}/interunit/transfer-in/${headerId}/acknowledge/${encodeURIComponent(boxId)}`, {
      method: 'DELETE'
    })
  }
  ...
  static async finalizeTransferIn(headerId: number, payload: {
    box_condition?: string
    condition_remarks?: string
    cold_storage_items?: any[]
  }): Promise<any> {
    return await fetchJSON(`${API_BASE_URL}/interunit/transfer-in/${headerId}/finalize`, {
      method: 'POST',
      body: JSON.stringify(payload)
    })
  }
