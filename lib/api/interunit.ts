...
export class InterUnitAPI {
  ...
  // 11. DELETE TRANSFER
  async delete(id: number | string, userEmail: string): Promise<DeleteResponse> {
    const params = new URLSearchParams()
    params.append('user_email', userEmail)
    return this.apiCall<DeleteResponse>(`/interunit/transfers/${id}?${params.toString()}`, {
      method: 'DELETE',
    })
  }

  // 7. APPROVE TRANSFER
  async approve(id: number | string, approvedBy: string): Promise<InterUnitTransfer> {
    const params = new URLSearchParams()
