...
export const ColdStorageApiService = {
  async searchColdStorageStocks(
    params: Record<string, string>
  ): Promise<{ results: ColdStorageStockRecord[]; total: number }> {
    const sp = new URLSearchParams(params)
    const qs = sp.toString() ? `?${sp.toString()}` : ""
    const response = await fetch(`${API_URL}/cold-storage/stocks/search${qs}`, {
      method: "GET",
      headers: getAuthHeaders(),
    })
    return handleResponse<{ results: ColdStorageStockRecord[]; total: number }>(response)
  },

  async pickBoxes(params: {
    company: string
    ...
    return handleResponse<{ boxes: { id: number; box_id: string; transaction_no: string; weight_kg: number }[] }>(response)
  },
}
