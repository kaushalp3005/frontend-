...
export class InwardFormUtils {
  /**
   * Check if any dropdown field value is "other" (case-insensitive)
   * Items with "other" values don't have real SKUs in the database
   */
  static isOtherItem(article: any): boolean {
    const fieldsToCheck = [
   ...
   */
  static async resolveSkuId(article: any, company: Company): Promise<number | null> {
    // Skip SKU lookup for "other" items - they don't have a real SKU in the database
    if (this.isOtherItem(article)) {
   ...
   * "Other" items will have sku_id set to null
   */
    ...
    const resolvedArticles = await Promise.all(
      articles.map(async (article) => {
        try {
          ...
          // For "other" items, skuId will be null - that's expected
  ...
  static validateFormData(formData: any, articles: any[], boxes: any[]): ValidationResult {
    ...
    // Validate SKU IDs (skip validation for "other" items)
    articles.forEach((article, index) => {
      const isOther = this.isOtherItem(article)
      const skuValidation = InwardFormValidator.validateSkuId(article.sku_id, isOther)
  ...
  static generateBoxes(articles: any[], existingBoxes: any[] = []): any[] {
    ...
    articles.forEach((article) => {
      // Only generate boxes if UOM is BOX, CARTON, or BAG and quantity is greater than 0
      if (article.quantity_units > 0 && (article.uom === "BOX" || article.uom === "CARTON" || article.uom === "BAG")) {
