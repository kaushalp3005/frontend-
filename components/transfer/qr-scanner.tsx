...
interface QRScannerProps {
  ...
  roiConfig?: {
    widthPercentage?: number  // Percentage of viewport width (default: 70)
    heightPercentage?: number // Percentage of viewport height (default: 70)
  }
}

export default function QRScanner({ onScanSuccess, onScanError, roiConfig }: QRScannerProps) {
        ...
        await scannerRef.current.start(
          { facingMode: "environment" }, // Try environment mode first
          {
            ...
            qrbox: function(viewfinderWidth, viewfinderHeight) {
              // Use ROI config or default to 70% of the smaller dimension
              const widthPercentage = (roiConfig?.widthPercentage || 70) / 100
              const heightPercentage = (roiConfig?.heightPercentage || 70) / 100

              const qrboxWidth = Math.floor(viewfinderWidth * widthPercentage)
              const qrboxHeight = Math.floor(viewfinderHeight * heightPercentage)
