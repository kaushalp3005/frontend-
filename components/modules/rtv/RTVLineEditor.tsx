...
import {
  Package, X, Loader2, Search, List, CheckCircle,
} from "lucide-react"
import { dropdownApi, type Company } from "@/lib/api"
import { cn } from "@/lib/utils"

export interface RTVLineForm {
...
interface RTVLineEditorProps {
  line: RTVLineForm
  index: number
  company: string
  onChange: (index: number, field: keyof RTVLineForm, value: string) => void
...
export function RTVLineEditor({
  ...
  // Track if item is "resolved" (selected via search or dropdown)
  ...
  // If line already has data from API (loaded from backend), mark as resolved
  useEffect(() => {
    if (line.item_description && (line.material_type || line.item_category)) {
      setIsResolved(true)
      onResolved?.(index)
    }
  }, [])
  ...
  const handleSelectSearchItem = (item: SearchResult) => {
    ...
    onChange(index, "sale_group", item.sale_group || "")
    if (item.uom) onChange(index, "uom", item.uom)
    setIsResolved(true)
    onResolved?.(index)
    setSearchQuery("")
    setShowResults(false)
  ...
  const handleItemDescriptionChange = async (value: string) => {
    onChange(index, "item_description", value)
    const match = itemDescriptions.find((d) => d.desc === value)
    if (match) {
      setIsResolved(true)
      onResolved?.(index)
      // Fetch sale_group from sku-id endpoint
      try {
        const skuData = await dropdownApi.fetchSkuId({
          company,
        ...
        if (skuData?.sale_group) {
          onChange(index, "sale_group", skuData.sale_group)
        }
        // Autofill UOM when backend returns it. Backend may name the field
        // "uom" (new sku endpoint) or skip it (older /inward/sku-id).
        const skuUom = (skuData as { uom?: string })?.uom
        if (skuUom) onChange(index, "uom", skuUom)
      } catch (err) {
        console.error("Failed to fetch sale_group:", err)
      }
    } else {
      setIsResolved(false)
    }
  ...
  return (
    <div className={cn(
      "p-3 sm:p-4 border rounded-lg bg-muted/20 space-y-3 overflow-visible",
    )}>
      {/* Header row */}
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs sm:text-sm font-medium">Line {index + 1}</span>
        </div>
      ...
      {isResolved ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2 p-2 sm:p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg">
            <div className="space-y-1 flex-1 min-w-0">
              <p className="text-xs sm:text-sm font-medium break-words">{line.item_description}</p>
              <div className="flex flex-wrap gap-1">
                {line.material_type && <Badge variant="outline" className="text-[10px] sm:text-xs">{line.material_type}</Badge>}
                {line.item_category && <Badge variant="outline" className="text-[10px] sm:text-xs">{line.item_category}</Badge>}
                {line.sub_category && <Badge variant="outline" className="text-[10px] sm:text-xs">{line.sub_category}</Badge>}
              </div>
            </div>
            {!disabled && (
              <Button
          ...
          {/* Read-only category fields */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Material Type</Label>
              <Input value={line.material_type || "\u2014"} readOnly className="h-8 sm:h-9 bg-muted text-xs" />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Category</Label>
              <Input value={line.item_category || "\u2014"} readOnly className="h-8 sm:h-9 bg-muted text-xs" />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Sub Category</Label>
              <Input value={line.sub_category || "\u2014"} readOnly className="h-8 sm:h-9 bg-muted text-xs" />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Sale Group</Label>
              <Input value={line.sale_group || "\u2014"} readOnly className="h-8 sm:h-9 bg-muted text-xs" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
            ...
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Rate</Label>
              <Input type="number" step="0.01" value={line.rate} onChange={(e) => onChange(index, "rate", e.target.value)} placeholder="0" className="h-8 sm:h-9 text-xs" disabled={disabled} />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Value</Label>
              <Input type="number" step="0.01" value={line.value} placeholder="Auto" className="h-8 sm:h-9 text-xs bg-muted" readOnly />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Carton Weight</Label>
        ...
        <div className="space-y-3">
          ...
          {mode === "search" && (
            <div className="relative" ref={wrapperRef}>
              ...
              {showResults && searchResults.length > 0 && (
                <div className="absolute z-50 left-0 right-0 mt-1 bg-popover border border-border rounded-lg shadow-lg flex flex-col" style={{ maxHeight: "min(480px, 60vh)" }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  disabled={!line.material_type || ddLoading === "item_category"}
                >
                  <SelectTrigger className="h-8 sm:h-9">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  disabled={!line.item_category || ddLoading === "sub_category"}
                >
                  <SelectTrigger className="h-8 sm:h-9">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  disabled={!line.sub_category || ddLoading === "item_description"}
                >
                  <SelectTrigger className="h-8 sm:h-9">
          ...
          {/* Numeric fields — always visible in input mode */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <Input type="number" step="0.01" value={line.rate} onChange={(e) => onChange(index, "rate", e.target.value)} placeholder="0" className="h-8 sm:h-9 text-xs" />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs">Value</Label>
              <Input type="number" step="0.01" value={line.value} readOnly placeholder="Auto" className="h-8 sm:h-9 text-xs bg-muted" />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs">Carton Weight</Label>
