...
export function BoxesTable({ boxes = [], onBoxesChange, headerTotals, articles = [] }: BoxesTableProps) {
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {(() => {
                        ...
                        return (
                          <Input
                            key={`count-${index}-${associatedArticle?.material_type || 'none'}-${associatedArticle?.uom || 'none'}`}
                            type="number"
                            min="1"
                            value={box.count || 1}
                            onChange={(e) => updateBox(index, "count", Math.max(1, Number.parseInt(e.target.value) || 1))}
                            onBlur={(e) => {
                              if (!e.target.value || Number(e.target.value) < 1) {
                                updateBox(index, "count", 1)
                              }
                            }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                type="number"
                min="1"
                value={newBox.count || 1}
                onChange={(e) => setNewBox((prev) => ({ ...prev, count: Math.max(1, Number.parseInt(e.target.value) || 1) }))}
                className="no-spinner"
                disabled
                title="Count is locked (only editable for PM material type)"
              />
            </div>
            <div>
