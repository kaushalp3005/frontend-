...
export function ArticleEditor({
  ...
  // ─── Search mode state ───────────────────────────
  const [searchQuery, setSearchQuery] = useState(article.item_description || "")
  ...
  // Auto-search on mount if article has a description from PO extraction but isn't resolved
  useEffect(() => {
    if (
      ...
      doSearch(article.item_description)
    }
  }, [article.item_description, article.sku_id, article.skuStatus, mode, doSearch])
  ...
  // Auto-compute total_amount = po_weight * unit_rate
  const computedTotal = (article.po_weight != null && article.unit_rate != null)
    ? article.po_weight * article.unit_rate
    : undefined

  // Sync total_amount whenever po_weight or unit_rate changes
  useEffect(() => {
    if (computedTotal != null) {
      const rounded = Math.round(computedTotal * 100) / 100
      if (article.total_amount !== rounded) {
        onChange(index, "total_amount", rounded)
      }
    }
  }, [article.po_weight, article.unit_rate])
  ...
  return (
    <div className={cn(
      ...
      article.skuStatus === "error" && "border-red-500 border-2 bg-red-50/30 dark:bg-red-950/20"
    )}>
      {/* Header row */}
      ...
      {isResolved ? (
        <div className="space-y-2">
          ...
          {/* Article-level amounts */}
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">PO Weight</Label>
              <Input
                type="number"
                value={article.po_weight ?? ""}
                onChange={(e) => onChange(index, "po_weight", e.target.value ? parseFloat(e.target.value) : undefined)}
                placeholder="0"
                className="h-8 sm:h-9 text-xs"
              />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Unit Rate</Label>
              <Input
                type="number"
                value={article.unit_rate ?? ""}
                onChange={(e) => onChange(index, "unit_rate", e.target.value ? parseFloat(e.target.value) : undefined)}
                placeholder="0.00"
                className="h-8 sm:h-9 text-xs"
              />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs text-muted-foreground">Total Amount</Label>
              <Input
                type="number"
          ...
          <p className="text-[10px] text-muted-foreground">
            PO Weight × Unit Rate = Total Amount
            {article.po_weight != null && article.unit_rate != null && (
              <span className="ml-1 text-foreground font-medium">
                ({article.po_weight} × {article.unit_rate} = {article.total_amount ?? 0})
              </span>
            )}
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          {mode === "search" && (
            <div className="relative" ref={wrapperRef}>
              <Label className="text-xs mb-1.5 block">Search Item Description <span className="text-destructive">*</span></Label>
          ...
          {mode === "dropdown" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              ...
              <div className="space-y-1 sm:col-span-2">
                <Label className="text-[10px] sm:text-xs">Item Description <span className="text-destructive">*</span></Label>
                <Select
          ...
          {/* UOM + Article amounts — always visible in input mode */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs">PO Weight</Label>
              <Input
                type="number"
                value={article.po_weight ?? ""}
                onChange={(e) => onChange(index, "po_weight", e.target.value ? parseFloat(e.target.value) : undefined)}
            ...
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs">Unit Rate</Label>
              <Input
                type="number"
                value={article.unit_rate ?? ""}
                onChange={(e) => onChange(index, "unit_rate", e.target.value ? parseFloat(e.target.value) : undefined)}
            ...
            <div className="space-y-1">
              <Label className="text-[10px] sm:text-xs">Total Amount</Label>
              <Input
                type="number"
                value={article.total_amount ?? ""}
                readOnly
